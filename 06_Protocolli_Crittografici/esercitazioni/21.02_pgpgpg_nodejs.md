...
- Garantire l'interoperabilità tra OpenPGP.js e il tool a riga di comando GPG

## Introduzione teorica

### OpenPGP.js e il suo ruolo

**OpenPGP.js** è l'implementazione JavaScript dello standard OpenPGP (RFC 4880), mantenuta da Proton (Proton Mail) e distribuita come pacchetto npm. È la stessa libreria usata internamente da ProtonMail per la crittografia lato client.

```
Standard OpenPGP (RFC 4880 / 4880bis)
          │
          ├── GnuPG (C)       → tool CLI per desktop/server
          ├── OpenPGP.js      → JavaScript / Node.js / Browser
          ├── Bouncy Castle   → Java / Kotlin
          └── GopenPGP        → Go
```

La libreria è importante perché consente di:
- Cifrare/decifrare **nel browser** senza trasmettere dati al server
- Integrare la crittografia nelle **REST API** Node.js
- Garantire **interoperabilità** con GPG desktop (stesso standard)
...
│                    CLIENT (browser/app)                      │
│                                                              │
│  [Genera coppia chiavi] → pubkey → [Carica sul server]       │
│                                                              │
│  Messaggio in chiaro                                         │
│       │                                                      │
│  [openpgp.encrypt(pubkey destinatario)]                      │
│       │                                                      │
│  Messaggio cifrato  ─────────────► POST /api/messages        │
└──────────────────────────────────────────────────────────────┘
                                          │
                                    [archiviato cifrato]
                                          │
...
│              Messaggio cifrato                               │
│                    │                                         │
│  [openpgp.decrypt(chiave privata)]                           │
│                    │                                         │
│              Messaggio in chiaro                             │
└──────────────────────────────────────────────────────────────┘
```
...
- Il server vede solo dati cifrati — non può leggere i messaggi
...
// ❌ Errato — la libreria non ha API sincrone
...
- **GPG** (per l'esercitazione 5 — interoperabilità)  
...
✅ Chiavi generate e salvate:

📄 chiave_pubblica.asc
🔒 chiave_privata.asc
⚠️  certificato_revoca.asc
...
async function ispezioneChiave() {
  ...
  console.log(`⏳ Scadenza:    ${scadenza ? scadenza.toLocaleDateString('it-IT') : 'Nessuna'}`);
...
👤 Utente:      Mario Rossi <mario.rossi@scuola.it>
🔑 Key ID:      A1B2C3D4E5F6A7B8
🖊️  Fingerprint: A1B2C3D4E5F6A7B8C9D0E1F2A3B4C5D6E7F8A9B0
📅 Creata:      01/01/2026
⏳ Scadenza:    Nessuna
🧮 Algoritmo:   eddsa (curve25519)
```
...
Testo originale: Ciao Mario! Questo messaggio è riservato. La password del server è: Tigre@2026
...
> ⚠️ Il file `messaggio_cifrato.asc` contiene solo dati incomprensibili: senza la chiave privata di Mario è impossibile leggerlo.
...
--- Contenuto ---
Ciao Mario! Questo messaggio è riservato. La password del server è: Tigre@2026

--- Info firma ---
ℹ️  Messaggio non firmato
```
...
✅ Messaggio firmato salvato in: comunicato_firmato.asc
...
✅ Il testo è leggibile anche da chi non usa OpenPGP/GPG. La firma è in fondo.
...
> ⚠️ **Prova a modificare** il file `comunicato_firmato.asc` (cambia anche solo una lettera) e poi riesegui la verifica: GPG.js rileverà la manomissione e riporterà `❌ FIRMA NON VALIDA`.
...
│   └── keystore.js        ← gestione chiavi in memoria
├── chiave_pubblica.asc
├── chiave_privata.asc
└── package.json
```
...
Endpoints disponibili:
  POST /api/keys    — registra chiave pubblica
  GET  /api/keys    — lista chiavi registrate
  POST /api/encrypt — cifra messaggio
  POST /api/decrypt — decifra messaggio
```
...
## Esercitazione Pratica 5 - Interoperabilità con GPG CLI

Uno dei punti di forza di OpenPGP.js è la piena compatibilità con GPG da riga di comando. Un messaggio cifrato da Node.js può essere decifrato con GPG e viceversa.

### Step 1: Importare la Chiave Node.js in GPG

```bash
# La chiave generata da OpenPGP.js è standard OpenPGP → si importa normalmente in GPG
gpg --import chiave_pubblica.asc
gpg --import chiave_privata.asc     # GPG chiederà la passphrase: SuperSegreto123!
...
✅ GPG riesce a decifrare un messaggio cifrato da OpenPGP.js — stessa chiave, stesso standard.
...
```
✅ Decifrato da Node.js: Risposta da GPG: confermato ricevuto.
```

✅ **Interoperabilità confermata**: GPG CLI ↔ OpenPGP.js funzionano senza problemi.
...
**Causa**: la stringa armored (`-----BEGIN PGP...`) è stata troncata, ha caratteri extra o newline errati (es. conversione JSON mal gestita).
...
const keyInJson = JSON.stringify(armoredKey);    // ✅ conserva \n come \\n
```
...
**Soluzione:** Non è possibile recuperare la passphrase. Controlla di non aver spazi o caratteri extra:
```javascript
// ✅ Usa .trim() per rimuovere spazi/newline accidentali
...
**Soluzione — Opzione A**: rimuovi `"type": "module"` da `package.json`.

**Soluzione — Opzione B**: usa la sintassi ES Module:
```javascript
...
### Esercizio 1: Generazione e Ispezione (★☆☆)
...
3. Confronta dimensione dei file `.asc` generati: quale è più grande?

**Domande:**
- Come si specifica `rsaBits: 4096` in `generateKey()`?
- Perché ECC produce chiavi più piccole di RSA pur garantendo sicurezza equivalente?

### Esercizio 2: Cifratura Multi-Destinatario (★★☆)
...
**Domande:**
- Come il formato PGP permette la decifratura a più destinatari mantenendo un solo ciphertext?
- Cosa succede se si tenta di decifrare con una terza chiave non inclusa?

### Esercizio 3: Middleware Express di Decifratura (★★☆)

1. Aggiungi al server Express un middleware che legge l'header `X-Encrypted-Payload`
2. Se l'header è presente, decifra automaticamente il corpo della richiesta prima di passarla al controller
...
### Esercizio 4: Firma e Verifica di un JSON (★★★)
...
### Esercizio 5: CLI Tool per Cifratura File (★★★)
...
- [OpenPGP.js – GitHub](https://github.com/openpgpjs/openpgpjs)
- [OpenPGP Standard – RFC 4880](https://datatracker.ietf.org/doc/html/rfc4880)
- [ProtonMail – Come usa OpenPGP.js](https://proton.me/blog/protonmail-open-source)

### Pacchetti npm Correlati
- [`openpgp`](https://www.npmjs.com/package/openpgp) – libreria principale usata in questa guida
- [`node-forge`](https://www.npmjs.com/package/node-forge) – crittografia generale in Node.js
- [`jose`](https://www.npmjs.com/package/jose) – JSON Web Tokens (JWE, JWS)
...
Hai imparato a:
✅ Installare e configurare OpenPGP.js in un progetto Node.js  
✅ Generare coppie di chiavi RSA ed ECC tramite codice  
✅ Cifrare e decifrare messaggi con API async/await  
✅ Firmare messaggi e verificarne l'autenticità  
✅ Costruire un server Express con endpoint REST per messaggistica cifrata  
✅ Garantire l'interoperabilità tra OpenPGP.js e GPG da riga di comando  

OpenPGP.js porta la crittografia PGP direttamente nelle applicazioni web e Node.js, permettendo di costruire sistemi di messaggistica **end-to-end encrypted** dove nemmeno il server può leggere i dati degli utenti. Questa è la base su cui sono costruiti servizi come ProtonMail e Signal.
