...
I servizi cloud come AWS S3 e Google Cloud Storage sono infrastrutture sicure, ma il provider detiene le chiavi di cifratura: tecnicamente potrebbe accedere ai tuoi dati. Con la crittografia **end-to-end (E2E)** applicata prima dell'upload, solo il proprietario della chiave privata può leggere i dati — accesso fisico al bucket non è sufficiente. In questa esercitazione costruirai un agente di backup Node.js che cifra ogni file con OpenPGP.js prima di caricarlo su cloud, e lo decifra localmente al momento del ripristino.
...
- Gestire manifest di backup cifrati per garantire integrità e tracciabilità
- Implementare il ripristino selettivo con verifica dell'integrità tramite hash SHA-256
...
```
┌───────────────────────────────────────────────────────────────┐
│               CIFRATURA LATO SERVER (SSE) — ⚠️ INSICURA E2E  │
│                                                               │
│  Il tuo file ──► HTTPS ──► [Provider riceve file in chiaro]   │
│                                    │                          │
│                              [Provider cifra]                 │
│                              con proprie chiavi               │
│                                    │                          │
...
│                              nel bucket                       │
│                                                               │
│  Chi può leggere: tu + provider + chi ottiene accesso al      │
│  pannello di controllo del provider                           │
└───────────────────────────────────────────────────────────────┘
```

Con **cifratura E2E lato client**, il file arriva al provider già cifrato:

```
┌───────────────────────────────────────────────────────────────┐
│            CIFRATURA E2E LATO CLIENT — ✅ SICURA              │
│                                                               │
│  Il tuo file ──► [OpenPGP.js cifra localmente]                │
│                          │                                    │
│                   File cifrato .pgp                           │
│                          │                                    │
│                    HTTPS upload                               │
...
│                  (non può leggere)                            │
│                                                               │
│  Chi può leggere: SOLO chi possiede la chiave privata         │
└───────────────────────────────────────────────────────────────┘
```
...
│                                                                     │
│  ┌──────────────┐    ┌───────────────┐    ┌─────────────────────┐   │
│  │  Scanner     │    │  Crypto Engine│    │  Storage Backend    │   │
│  │  (fs.readdir)│───►│  (OpenPGP.js) │───►│  Local / S3 / GCS   │   │
│  └──────────────┘    └───────────────┘    └─────────────────────┘   │
│         │                    │                       │              │
│         │              session key                   │              │
│         │            AES-256 per file                │              │
...
│  ┌──────▼────────────────────▼───────────────────────▼───────────┐  │
│  │                      MANIFEST (cifrato)                       │  │
│  │  { file, hash_sha256, timestamp, storage_path, key_id }       │  │
│  └───────────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────────┘
```
...
PGP usa già internamente la **crittografia ibrida**:

```
Per ogni file da backuppare:

  1. Genera session key casuale (AES-256, 256 bit)
  2. Cifra il file con la session key  ──► file.pgp (contenuto)
  3. Cifra la session key con RSA/ECC  ──► session key cifrata
  4. Salva entrambi nel file .pgp finale

Solo il proprietario della chiave privata può:
...
bucket-backup/
├── manifests/
│   └── backup_20260226_143000.json.pgp   ← manifest cifrato
├── data/
│   ├── a3f8c1d2e4b5.pgp                  ← file cifrato (nome randomizzato)
│   ├── 7b2e9f4a1c3d.pgp
│   └── ...
└── keys/
    └── pubkey_backup.asc                 ← chiave pubblica (non segreta)
```

> ⚠️ I nomi dei file sono randomizzati per non rivelare la struttura delle directory anche a chi accede fisicamente al bucket.
...
- **GPG** installato (opzionale, per verifica interoperabilità)
- Account **AWS** con bucket S3 (solo per Esercitazione 3 – parte S3)

> ⚠️ Le esercitazioni 1, 2, 4 e 5 usano uno storage backend locale e **non richiedono** account cloud. AWS S3 è introdotto come opzione nella Pratica 3.
...
backup-e2e/
├── src/
│   ├── crypto.js        ← motore crittografico
│   ├── storage.js       ← backend storage (locale + S3)
│   ├── manifest.js      ← gestione manifest cifrato
│   ├── backup.js        ← agente di backup
│   └── restore.js       ← agente di ripristino
├── keys/                ← chiavi GPG (NON caricare su git!)
├── backups/local/       ← storage locale simulato
├── test-data/           ← file di test
...
async function generaChiaviBackup() {
  ...
  console.log(`\n🔑 Generazione chiavi di backup per: ${nome} <${email}>\n`);

  const passphrase = await chiediPassphrase();
  if (passphrase.length < 12) {
    console.error('❌ La passphrase deve essere di almeno 12 caratteri.');
    process.exit(1);
  }

  console.log('\n⏳ Generazione in corso (ECC Curve25519)...');
  ...
  console.log('\n✅ Chiavi generate e salvate in keys/');
  console.log('   backup_pub.asc   — chiave pubblica (puoi condividerla)');
  console.log('   backup_priv.asc  — chiave PRIVATA  (tenere offline e sicura!)');
  console.log('   revocation.asc   — certificato di revoca (conservare offline)');
  console.log('\n⚠️  La passphrase è stata salvata in .env');
...
Crea il file `.env` (aggiungi le variabili base — la passphrase verrà aggiunta da `keygen.js`):
...
// ──────────────────────────────────────────────
// Calcola lo SHA-256 di un file (per verifica integrità)
// ──────────────────────────────────────────────
...
async function cifraFile(inputPath, outputPath, publicKey) {
  console.log(`  🔐 Cifratura: ${path.basename(inputPath)}`);

  // Calcola hash del file originale (per verifica integrità al ripristino)
  const originalHash = await calcolaHashFile(inputPath);

  // Leggi il file (per file grandi usare stream — vedi modulo avanzato)
  ...
  const encrypted = await openpgp.encrypt({
    ...
    format: 'armored',       // output ASCII → più facile da gestire
  });

  fs.writeFileSync(outputPath, encrypted);
  const encryptedSize = fs.statSync(outputPath).size;

  console.log(`  ✅ Cifrato → ${path.basename(outputPath)} (${formatSize(encryptedSize)})`);
...
async function decifraFile(encryptedPath, outputPath, privateKey, expectedHash = null) {
  console.log(`  🔓 Decifratura: ${path.basename(encryptedPath)}`);
  ...
  // Verifica integrità se è stato fornito l'hash atteso
  const decryptedHash = await calcolaHashFile(outputPath);

  if (expectedHash) {
    if (decryptedHash === expectedHash) {
      console.log(`  ✅ Integrità verificata: hash SHA-256 corrispondente`);
    } else {
      console.error(`  ❌ ATTENZIONE: hash SHA-256 NON corrispondente!`);
      console.error(`     Atteso:   ${expectedHash}`);
      console.error(`     Ottenuto: ${decryptedHash}`);
      throw new Error(`Integrità compromessa per il file ${path.basename(outputPath)}`);
    }
  }
...
async function testCrypto() {
  ...
  console.log(`\n${contenutoOriginale === contenutoRipristinato ? '✅ TEST SUPERATO' : '❌ TEST FALLITO'}`);
}
...
✅ Chiavi caricate
  ...
  🔐 Cifratura: report_gennaio.txt
  ✅ Cifrato → test_report.pgp (1.8 KB)
  Hash originale: 3a4f8b...

  🔓 Decifratura: test_report.pgp
  ✅ Integrità verificata: hash SHA-256 corrispondente
...
class LocalStorage {
  constructor(baseDir) {
    ...
    console.log(`📁 Storage locale: ${path.resolve(baseDir)}`);
  }
  ...
  async upload(localFile, remotePath) {
    ...
    console.log(`  📤 Upload locale: ${remotePath}`);
    return remotePath;
  }
  ...
  async download(remotePath, localFile) {
    ...
    console.log(`  📥 Download locale: ${remotePath}`);
    return localFile;
  }
  ...
  async delete(remotePath) {
    ...
    console.log(`  🗑️  Eliminato: ${remotePath}`);
  }
}
...
class S3Storage {
  constructor(bucketName, region = 'eu-west-1') {
    ...
    console.log(`☁️  Storage S3: s3://${bucketName} (${region})`);
  }
  ...
  async upload(localFile, remotePath) {
    ...
    console.log(`  📤 Upload S3: s3://${this.bucket}/${remotePath}`);
  ...
  async download(remotePath, localFile) {
    ...
    console.log(`  📥 Download S3: ${remotePath}`);
    return localFile;
  }
  ...
  async delete(remotePath) {
    ...
    await this.client.send(cmd);
    console.log(`  🗑️  Eliminato da S3: ${remotePath}`);
  }
}
...
Il **manifest** è un file JSON che registra tutti i file del backup con metadati e hash. È cifrato prima del caricamento.
...
async function eseguiBackup(sourceDir, opzioni = {}) {
  ...
  console.log('\n╔══════════════════════════════════════╗');
  console.log('║        AGENTE DI BACKUP E2E          ║');
  console.log('╚══════════════════════════════════════╝\n');
  console.log(`📂 Sorgente: ${path.resolve(sourceDir)}`);
  ...
  console.log('╠══════════════════════════════════════╣');
  console.log(`║  ID:      ${backupId.padEnd(27)}║`);
  console.log(`║  ✅ OK:   ${String(filesOk).padEnd(27)}║`);
  console.log(`║  ❌ Err:  ${String(filesErr).padEnd(27)}║`);
  console.log(`║  ⏱️  Tempo: ${elapsed}s${' '.repeat(24 - elapsed.length)}║`);
  console.log('╚══════════════════════════════════════╝\n');
  console.log(`📋 Manifest: ${manifestRemote}\n`);
...
```
╔══════════════════════════════════════╗
║        AGENTE DI BACKUP E2E          ║
╚══════════════════════════════════════╝

📂 Sorgente: /ws/backup-e2e/test-data
📋 File trovati: 4

  🔐 Cifratura: report_gennaio.txt
  ✅ Cifrato → b3a1f2e4c5d6.pgp (1.8 KB)
  📤 Upload locale: data/b3a1f2...pgp

  🔐 Cifratura: credenziali.txt
  ✅ Cifrato → 9d7e2b4a1f3c.pgp (1.9 KB)
  📤 Upload locale: data/9d7e2b...pgp
  ...

╔══════════════════════════════════════╗
║           BACKUP COMPLETATO          ║
╠══════════════════════════════════════╣
║  ID:      backup_20260226_143512     ║
║  ✅ OK:   4                         ║
║  ❌ Err:  0                         ║
║  ⏱️  Tempo: 1.3s                    ║
╚══════════════════════════════════════╝
...
## Esercitazione Pratica 5 - Ripristino e Verifica Integrità
...
async function eseguiRipristino(manifestRemote, destDir, opzioni = {}) {
  ...
  if (fileDaRipristinare.length === 0) {
    console.log('⚠️  Nessun file corrispondente trovato nel manifest.');
    return;
  }
  ...
  for (const entry of fileDaRipristinare) {
    try {
      ...
      // Decifra con verifica integrità (hash SHA-256)
      await crypto.decifraFile(tmpEncrypted, outputPath, privKey, entry.hash);

      // Pulisci
      fs.unlinkSync(tmpEncrypted);

      console.log(`  → Ripristinato: ${entry.originalPath}`);
      ok++;
    } catch (e) {
      console.error(`  ❌ Errore ripristino ${entry.originalPath}: ${e.message}`);
      err++;
    }
  }
  ...
  console.log('╠══════════════════════════════════════╣');
  console.log(`║  ✅ OK:   ${String(ok).padEnd(27)}║`);
  console.log(`║  ❌ Err:  ${String(err).padEnd(27)}║`);
  console.log(`║  ⏱️  Tempo: ${elapsed}s${' '.repeat(24 - elapsed.length)}║`);
  console.log(`║  📂 Dest: ${destDir.slice(0,27).padEnd(27)}║`);
  console.log('╚══════════════════════════════════════╝\n');
}
...
```
╔══════════════════════════════════════╗
║       AGENTE DI RIPRISTINO E2E        ║
╚══════════════════════════════════════╝

Passphrase chiave privata: ************
✅ Chiave privata sbloccata

📋 Scarico manifest: manifests/backup_20260226_143512_manifest.json.pgp

📦 Backup: backup_20260226_143512
   Creato: 26/02/2026, 14:35:12
   File totali: 4
   Etichetta: Backup lezione SIS3

  📥 Download locale: data/b3a1f2...pgp
  🔓 Decifratura: b3a1f2...pgp
  ✅ Integrità verificata: hash SHA-256 corrispondente
  → Ripristinato: report_gennaio.txt
  ...
...
✅ Hash identici — il file è stato ripristinato senza alcuna modifica.
...
console.log('╔══════════════════════════════════════╗');
console.log('║      BACKUP SCHEDULER AVVIATO        ║');
console.log('╠══════════════════════════════════════╣');
console.log(`║  Sorgente:  ${CONFIG.sourceDir.padEnd(27)}║`);
console.log(`║  Schedule:  ${CONFIG.schedule.padEnd(27)}║`);
console.log(`║  Storage:   ${CONFIG.storageConf.backend.padEnd(27)}║`);
console.log('╚══════════════════════════════════════╝\n');

// Esegui subito il primo backup (per verifica)
(async () => {
  console.log('▶️  Eseguendo backup iniziale...');
...
cron.schedule(CONFIG.schedule, async () => {
  console.log(`\n⏰ Backup pianificato avviato: ${new Date().toLocaleString('it-IT')}`);
  try {
    await eseguiBackup(CONFIG.sourceDir, {
    ...
    console.error('❌ Backup pianificato fallito:', err.message);
...
console.log('🕐 Scheduler attivo. Premi Ctrl+C per uscire.\n');
```
...
### Step 2: Retention Policy — Eliminare i Backup Vecchi

```javascript
// src/retention.js
// Mantiene solo gli N backup più recenti, elimina i più vecchi
...
async function applicaRetentionPolicy(maxBackup = 7, storageConf = {}) {
  ...
  // Ordina per nome (timestamp nel nome → ordine cronologico)
  const manifests = oggetti
    .filter(f => f.endsWith('.pgp'))
    .sort()
    .reverse();  // dal più recente al più vecchio

  console.log(`\n🗂️  Retention policy: mantieni ${maxBackup} backup, trovati ${manifests.length}\n`);
...
# → vede solo testo cifrato incomprensibile ✅
...
node -e "
  ...
  openpgp.readPrivateKey({ armoredKey: fs.readFileSync('keys/backup_priv.asc','utf8') })
    ...
    .then(() => console.log('✅ Chiave sbloccata'))
    .catch(e => console.error('❌', e.message));
"
```

#### 2. `Error: ENOENT` — file non trovato al ripristino
...
#### 3. `Error: Integrity compromessa` — hash non corrispondente

**Causa**: il file è stato corrotto o modificato durante il trasferimento/storage.
...
### Esercizio 1: Backup di un Intero Progetto (★☆☆)
...
**Domande:**
- Perché i nomi dei file nel bucket sono randomizzati anziché usare il nome originale?
- Quali informazioni riesce a ricavare un attaccante che accede al bucket, pur non potendo decifrare i file?

### Esercizio 2: Verifica Integrità dopo Manomissione (★★☆)
...
**Domande:**
- Cosa garantisce l'hash SHA-256 nel manifest che non garantisce già OpenPGP?
- Un hash SHA-256 nel manifest firmato digitalmente offrirebbe garanzie aggiuntive?

### Esercizio 3: Backup Differenziale (★★★)

Modifica l'agente di backup per supportare la modalità **differenziale**: carica solo i file modificati dall'ultimo backup.
...
for (const file of files) {
  ...
  if (fileGiaBackuppati.get(relativePath) === hashCorrente) {
    console.log(`  ⏭️  Invariato, salto: ${relativePath}`);
...
- Quali complessità aggiuntive introduce in fase di ripristino?

### Esercizio 4: Doppia Chiave — Backup e Recovery Agent (★★★)

In un'azienda reale il backup è gestito da un agente automatico, ma il ripristino può essere richiesto solo da un amministratore:
...
**Domande:**
- Perché l'agente di backup non ha mai bisogno della chiave privata?
- Cosa succederebbe se la chiave privata dell'agente di backup venisse compromessa?

### Esercizio 5: Notifiche di Backup (★★☆)
...
3. Invia l'email **dopo** la cifratura — il corpo dell'email non deve contenere nomi di file o percorsi (solo statistiche aggregate)

**Domande:**
- Perché è importante che le notifiche non rivelino nomi di file o percorsi?
...
- [OpenPGP.js – Documentazione](https://openpgpjs.org/)
...
- [node-cron – Documentazione](https://github.com/node-cron/node-cron)
...
- Monitora i backup: una notifica assente è un segnale di problema
- Testa periodicamente il ripristino — un backup mai testato non è un backup reale
...
Hai imparato a:
✅ Comprendere la differenza tra SSE (server-side) e cifratura E2E lato client  
✅ Costruire un motore crittografico con OpenPGP.js per file di qualsiasi dimensione  
✅ Implementare un'astrazione storage che supporta backend locale e AWS S3  
✅ Gestire un manifest cifrato come registro affidabile dei file backuppati  
✅ Verificare l'integrità dei file ripristinati tramite hash SHA-256  
✅ Schedulare backup automatici con politica di retention  
✅ Analizzare le minacce residue (metadata, nomi file, timing)  

La crittografia E2E applicata ai backup è il principio architetturale alla base di servizi come **Tresorit**, **SpiderOak** e **ProtonDrive**: il cloud diventa uno storage untrusted su cui appoggiarsi senza affidargli le proprie chiavi.
